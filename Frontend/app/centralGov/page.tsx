...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setCandidateVotes(validatorsData?.candidateVotes || null);
    } catch (err) {
  ...
  {transactions?.mainChain?.length > 0 ? (
    <div className="space-y-6">
      {transactions.mainChain
        .flatMap((block) => block.transactions || []) // Extract transactions
        .slice(-3) // Get the last 3 transactions
        .reverse() // Show newest first
        .map((tx, txIndex) => (
          <Card key={txIndex} className="border-l-4 border-l-green-600 shadow-md">
            <CardContent className="p-4">
              <div className="flex justify-between items-center">
                <div className="text-sm font-medium">
                  <span className="text-gray-700 dark:text-gray-300">From:</span> {tx.sender}
                </div>
                <ArrowRight className="h-4 w-4 text-green-600" />
                <div className="text-sm font-medium">
                  <span className="text-gray-700 dark:text-gray-300">To:</span> {tx.receiver}
                </div>
              </div>
    ...
    <p className="text-gray-500 text-center py-6">No recent transactions found in the main branch.</p>
  )}
</CardContent>
  ...
  {transactions?.mainChain?.length > 0 ? (
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-1 gap-3 mt-2">
                {block.transactions?.length > 0 ? (
                  ...
                  <p className="text-gray-500 text-center">No transactions in this block.</p>
                )}
              </div>
    ...
    <p className="text-gray-500 text-center py-6">No transactions found in the main branch.</p>
  )}
</CardContent>
